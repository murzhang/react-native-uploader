...
public class FileUploadModule extends ReactContextBaseJavaModule {
    ...
    public void upload(final ReadableMap options, final Callback callback) {
        ...
        String uploadUrl = options.getString("url");
        String method;
        ...
        ReadableMap fields = options.getMap("params");
        ...
        try {
            ...
            for (int i = 0; i < files.size(); i++) {
				...
				long total = bytesAvailable;
				long progressBarLength = 0;
                while (bytesRead > 0) {
                    outputStream.write(buffer, 0, bufferSize);

					//获取进度
					progressBarLength += bytesRead;
					float progress=((progressBarLength / (float) total) * 100);
					WritableMap params = Arguments.createMap();  
					params.putString("fileIndex",String.valueOf(i));
					params.putString("files",String.valueOf(files.size()));
					params.putString("progress",String.valueOf(progress));
